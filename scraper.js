...
async function fetchTalentsPage(page, location = 'united_states_only') {
  const url = `${BASE_URL}/search-talent/?custom_location=${location}&page=${page}`;
  console.log(`Fetching page ${page} from: ${url}`);
  
  try {
    const response = await makeRequest(url);
    console.log(`Page ${page} response:`, { status: response.status, hasData: !!response.data });
    
    if (response.status === 200 && response.data) {
      console.log(`Page ${page} data structure:`, {
      ...
      console.error(`Error fetching page ${page}: Status ${response.status}`, response.data);
      return null;
    }
  } catch (error) {
...
async function scrapeTalents(startPage, endPage) {
  ...
  while (currentPage <= endPage) {
    ...
    console.log(`Page ${currentPage} fetch completed. Data:`, pageData ? 'received' : 'null');
    
    if (!pageData) {
      console.log(`No data returned for page ${currentPage}, stopping...`);
      break;
    }
    
    if (!pageData.results || !Array.isArray(pageData.results)) {
      console.log(`No results array found for page ${currentPage}. PageData keys:`, Object.keys(pageData || {}));
